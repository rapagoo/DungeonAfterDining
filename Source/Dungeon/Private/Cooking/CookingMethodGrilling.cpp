...
{
	UE_LOG(LogTemp, Log, TEXT("UCookingMethodGrilling::ProcessIngredients called with %d ingredients"), Ingredients.Num());
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("No ingredients provided for grilling."));
		return false;
	}
	...
	if (ItemDataTable)
	{
		for (const FName& IngredientID : Ingredients)
		{
			...
			{
				FInventoryItemStruct* ItemData = ItemDataTable->FindRow<FInventoryItemStruct>(IngredientID, TEXT("GrillingCheckCookingUsability"));
				if (ItemData && !ItemData->bCanBeUsedInCooking)
				{
					UE_LOG(LogTemp, Warning, TEXT("UCookingMethodGrilling: Ingredient '%s' is improperly prepared and cannot be used for grilling"), 
						   *IngredientID.ToString());
					return false;  // Grilling failed due to improperly prepared ingredient
				}
			}
		}
	}
