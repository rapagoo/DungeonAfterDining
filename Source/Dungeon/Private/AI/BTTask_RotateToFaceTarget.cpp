...
FString UBTTask_RotateToFaceTarget::GetStaticDescription() const
{
	...
	return FString::Printf(TEXT("Smoothly rotates to face %s Key untill the angle precision: %s is reached"), *KeyDescription, *FString::SanitizeFloat(AnglePrecision));
}
...
void UBTTask_RotateToFaceTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	else
	{
		...
		const FRotator TargetRot = FMath::RInterpTo(Memory->OwningPawn->GetActorRotation(), LookAtRot, DeltaSeconds, RotationiInterpSpeed);
